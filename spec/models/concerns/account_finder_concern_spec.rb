...
describe AccountFinderConcern do
  ...
  describe 'remote finders' do
    before do
      @account = Fabricate(:account, username: 'Alice', domain: 'etherhive.io')
    end

    describe '.find_remote' do
      it 'returns exact match result' do
        expect(Account.find_remote('alice', 'etherhive.io')).to eq(@account)
      end
      ...
      it 'returns nil when username does not match' do
        expect(Account.find_remote('a_ice', 'etherhive.io')).to be_nil
      end

      it 'returns nil when domain does not match' do
        expect(Account.find_remote('alice', 'e_therhive.io')).to be_nil
      end
    ...
    describe '.find_remote!' do
      ...
      it 'raises on non-matching result' do
        expect { Account.find_remote!('missing', 'etherhive.host') }.to raise_error(ActiveRecord::RecordNotFound)
      end
